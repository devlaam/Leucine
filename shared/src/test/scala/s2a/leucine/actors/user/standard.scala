...
object StandardActorTest extends TestSuite :
  ...
  class Joni(val writeln: String => Unit) extends StandardActor[Joni.Letter] :
    val name = "Joni"
    ...
    def receive(letter: Joni.Letter, sender: Sender) = (letter,sender) match
      ...
      case (Joni.Text(text,depth), _: Joni)  => writeln(s"$text$depth")
  ...
  class Mary(val writeln: String => Unit) extends StandardActor[Mary.Letter] :
    val name = "Mary"
    ...
    def receive(letter: Mary.Letter, sender: Sender) = (letter,sender) match
      ...
      case (Mary.Text(text,depth), _: Mary)  => writeln(s"$text$depth")
  ...
  class Sara(val writeln: String => Unit) extends StandardActor[Sara.Letter] :
    val name = "Sara"
    var joni: Option[Joni] = None
    var mary: Option[Mary] = None
    ...
    def receive(letter: Sara.Letter, sender: Sender) = (letter,sender) match
      ...
      case (Sara.Text(text,depth), _: Sara)  => writeln(s"$text$depth")
  ...
  val tests = Tests {
    ...
    def readlns: Set[String] = synchronized{result.toSet}
    test("sending looped letters"){
      val joni = Joni(writeln)
      val mary = Mary(writeln)
      val sara = Sara(writeln)
