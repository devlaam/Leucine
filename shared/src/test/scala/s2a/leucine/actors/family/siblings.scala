...
object ActorSiblingNoRelaySupply extends TestSuite :
  import TestMethods.*
  ...
  class Mama(name: String, val parent: Grandma) extends RestrictActor(Mama,name), FamilyBranch[Grandma,Mama.type](Mama) :
    /* Make some sons and daughters. */
    new Son("s1",this)
    new Son("s2",this)
    new Son("s3",this)
    new Daughter("d1",this)
    new Daughter("d2",this)
    new Daughter("d3",this)
    def receive[Sender <: Accept](letter: Letter[Sender], sender: Sender): Unit = letter match
      case Mama.Hello =>
...
object ActorSiblingDoRelaySupply extends TestSuite :
  import TestMethods.*
  implicit val ac: ActorContext = ActorContext.system
