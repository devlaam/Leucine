...
object ActorLoggerSortTest extends TestSuite :
  ...
  import ActorLogger.{Level, Entry, Timing, Channel, Filter, Capture}
  ...
  val tests = Tests :
    ...
    test("ActorLogger.sort sorts randomly dispersed two-level lists") :
      ...
      def fillGroup(nr: Int): Entry =
        val entry = ActorLogger.Entry("parent.child",Timing.Recent,Capture(Level.Info,Channel.Pass,Filter.Pass,Static.Class,s"Class$nr",s"msg-$nr"))
    ...
    test("ActorLogger.sort ignores empty inner lists between populated lists") :
      val first  = ActorLogger.Entry("parent.child",Timing.Recent,Capture(Level.Warn,Channel.Pass,Filter.Pass,Static.Class,"ClassA","first"))
      val second = ActorLogger.Entry("parent.child",Timing.Recent,Capture(Level.Warn,Channel.Pass,Filter.Pass,Static.Class,"ClassB","second"))
...
object ActorLoggerStichTest extends TestSuite :
  ...
  import ActorLogger.{Entry, Level, Timing, Channel, Capture, Filter}
    ...
    test("stichedSpool composes random hold stream into dense sorted output") :
      (1 to 100).foreach : seed =>
        ...
        val allEntries = (1 to sampleSize-1).map : nr =>
          ActorLogger.Entry("parent.child",Timing.Recent,Capture(Level.Info,Channel.Pass,Filter.Pass,Static.Method,s"Source.$nr",s"message-$nr"))
