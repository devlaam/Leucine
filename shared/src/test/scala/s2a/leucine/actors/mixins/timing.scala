...
trait TimingAidTest(using ac: ActorContext) :
  ...
  class Clock(withDump: Boolean, writeln: String => Unit, done: () => Unit) extends BasicActor(Clock), TimingAid :
  ...
  class Expect(event: => Boolean, writeln: String => Unit, done: () => Unit) extends BasicActor(Expect), TimingAid :
