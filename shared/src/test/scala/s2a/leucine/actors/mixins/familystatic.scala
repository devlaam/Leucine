...
object ActorFamilySupply extends TestSuite :
  ...
  class Outside extends StandardActor(Outside_,"boo"), TimingAid :
    ...
    def receive[T <: Sender](letter: Outside_.Letter[T], sender: T) = (letter,sender) match
  ...
  abstract class Level0 extends StandardActor(Level0_,"l0"), FamilyRoot(Level0_) :
    ...
    def receive[T <: Sender](letter: MyLetter[T], sender: T) = (letter,sender) match
  ...
  val tests = Tests {
    ...
    class Level0Ext(val outside: Outside) extends Level0 :
      ...
      override def receive[T <: Sender](letter: MyLetter[T], sender: T) = super.receive(letter,sender)
