...
object ActorFamilySupply extends TestSuite :
  ...
  class Outside extends StandardActor(Outside_,"boo"), TimingAid :
    ...
    def receive[T >: Common <: Sender](letter: Outside_.Letter[T], sender: T) = (letter,sender) match
  ...
  val tests = Tests {
    ...
    class Level0Ext(val outside: Outside) extends Level0 :
      ...
      override def receive[T >: Common <: Sender](letter: MyLetter[T], sender: T) = super.receive(letter,sender)
